// intervalWorkoutResult.ts
import { ChatInputCommandInteraction, SlashCommandBuilder } from "discord.js";
import { WorkoutName } from "@prisma/client";
import { INTERVAL_WORKOUT_CHOICES, ERROR_MESSAGES } from "../../../config/constants";
...
async function execute(interaction: ChatInputCommandInteraction) {
	try {
		...
		const reply = await processIntervalWorkout(attachment, workoutName, discordId);
		await interaction.editReply(reply);
	} catch (error) {
		console.error(error);
		await interaction.editReply(ERROR_MESSAGES.UNEXPECTED_ERROR);
	}
}

export default { ...insertIntervalWorkoutResult, execute };
